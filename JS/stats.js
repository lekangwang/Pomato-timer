...
function updateGraph(modeStr) {
  ...
  if (modeStr == "work") {
    ...
    if (currentPercentage > 100) {
      ...
      document.getElementById("stats-work-bar").style.height =
        currentPercentage + addPercentage + "%";
    }
    updateSessions("work");
  } else {
    let breakBarHeight = document.getElementById("stats-break-bar")
      .offsetHeight;
    currentPercentage = (breakBarHeight / maxBarHeight) * 100;
    if (currentPercentage > 100) {
      document.getElementById("stats-break-bar").style.height = 0 + "%";
    } else {
      document.getElementById("stats-break-bar").style.height =
        currentPercentage + addPercentage + "%";
    }
